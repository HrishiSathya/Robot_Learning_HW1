...
class SOPHIE():
    ...
    def get_feedback(self, env, goal, traj):
        ...
        dist_feedback = open("learning_implement-main/planning/joint_space_full_traj_dist/shortest_path_env_"+str(env)+"_goal_"+str(goal)+"_traj_"+str(traj)+".txt", "r")

        return dist_feedback
    ...
    def get_dist(self, dist, current_iter):

        return dist[current_iter]

    def store_Qtable(self, Qtable):
        ...
        for e in envs:
            for g in goals:
                for t in trajs:
                    ...
                    Dist = self.get_feedback(e, g, t)
                    self.action = np.array([-.1,-.1,-.1])
                    T = Trajectory.readlines()
                    dist_vals = [float(d.strip()) for d in Dist.readlines()]
    ...
    def exec(self):
        ...
        for _ in range(10):
            ...
            h = float(input("Update given dist: %s : " % np.linalg.norm(goal - state)))
            if h != 0:
...
sophie = SOPHIE()
sophie.Qtraining(sophie.rand_possible_env, 
                sophie.rand_possible_goal, 
                sophie.rand_possible_traj)
print(sophie.Qtable)
loss = sophie.exec()

np.savetxt("loss_sophie.txt", loss, delimiter=',')